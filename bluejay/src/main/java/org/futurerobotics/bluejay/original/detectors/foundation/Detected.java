...
class Detected {
	...
	double circularity(MatOfPoint inp) {
		...
		return bb.width / (double) bb.height;
	...
	public void draw(Mat canvas) {
		...
		Imgproc.drawContours(canvas, Collections.singletonList(shape), 0, color, 2);
		Imgproc.putText(canvas, c.toString(), new Point(bounds.tl().x, bounds.tl().y + 20), Core.FONT_HERSHEY_SIMPLEX,
				0.6, black, 7);
		Imgproc.putText(canvas, c.toString(), new Point(bounds.tl().x, bounds.tl().y + 20), Core.FONT_HERSHEY_SIMPLEX,
				0.6, color, 2);
